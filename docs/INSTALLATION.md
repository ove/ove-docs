...
* `MariaDB__ConnectionString` - The connection string used by the [Asset Manager](https://github.com/ove/ove-asset-services/tree/master/packages/ove-asset-manager) to connect to [MariaDB](https://mariadb.org/) database. If you have changed the values of the enivronment variables `MYSQL_DATABASE`, `MYSQL_USER`, or `MYSQL_PASSWORD` from their defaults then you will need to replace the values for `Database`, `User`, or `Password` specified in this string with their modified values. 
