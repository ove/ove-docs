...
An OVE `application` (or `app`) includes server-side and client-side components. Each of the [OVE Apps](https://github.com/ove/ove-apps) confirm to a [common structure](APP_DEVELOPMENT.md/#application-structure) and provide a way to display a distinct form of commonly used content (subject to the limitations described in the [list of pitfalls to avoid](PITFALLS.md)).
